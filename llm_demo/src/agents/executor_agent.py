...
class ExecutorAgent(BaseAgent):
    ...
    def _materialized_mv_record(self, candidate: dict[str, Any], batch_id: int, build_sql_path: Path) -> dict[str, Any]:
        return {
            ...
            "family_type": candidate.get("family_type"),
            "target_queries": candidate.get("target_queries", []),
            "source_query_ids": candidate.get("source_query_ids", []),
            "source_qb_ids": candidate.get("source_qb_ids", []),
            "target_qb_ids": candidate.get("target_qb_ids", []),
            "depends_on_mv_ids": candidate.get("depends_on_mv_ids", []),
            "mv_type": candidate.get("mv_type"),
            "mv_predicates": candidate.get("mv_predicates", []),
            "generalized_predicates": candidate.get("generalized_predicates", []),
            "residual_filters": candidate.get("residual_filters", []),
